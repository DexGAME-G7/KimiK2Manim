#!/usr/bin/env python3
"""Run the full kimi2k thinking pipeline on Torus Eigenfunctions task."""
...
async def run_pipeline(concept: str, prompt_text: str):
    ...
    logger.info("=" * 70)
    logger.info(f"RUNNING KIMI2K THINKING PIPELINE FOR: {concept}")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    prompt_file = script_dir / "instructions.txt"
    
    if not prompt_file.exists():
        print(f"ERROR: Instructions file not found: {prompt_file}")
        sys.exit(1)
    
    prompt_text = prompt_file.read_text(encoding="utf-8").strip()
    
    # Extract main concept - use "Torus Eigenfunctions" as the core concept
    concept = "Torus Eigenfunctions and Standing Wave Modes"
